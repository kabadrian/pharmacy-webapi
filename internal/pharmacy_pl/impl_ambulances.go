...
import (
	...
	"github.com/kabadrian/pharmacy-webapi/internal/db_service"
)

// Kópia zakomentovanej časti z api_ambulances.go
// CreateAmbulance - Saves new ambulance definition
func (api *implAmbulancesAPI) CreateAmbulance(ctx *gin.Context) {
...
func (api *implAmbulancesAPI) DeleteAmbulance(ctx *gin.Context) {
	value, exists := ctx.Get("db_service")
	if !exists {
		ctx.JSON(
			http.StatusInternalServerError,
