...
func NewMongoService[DocType interface{}](config MongoServiceConfig) DbService[DocType] {
	...
	if svc.DbName == "" {
		svc.DbName = enviro("AMBULANCE_API_MONGODB_DATABASE", "pharmacy-pl")
	}

	if svc.Collection == "" {
		svc.Collection = enviro("AMBULANCE_API_MONGODB_COLLECTION", "pharmacy")
	}
...
func (this *mongoSvc[DocType]) GetDocuments(ctx context.Context) ([]DocType, error) {
	...
	var documents []DocType // Initialized but empty slice
